    ...
    with engine.connect() as conn:
        ...
        for d in range(1, delta_days):
            ...
            INSERT INTO {table_name} (
                ...
                new_clicked_bots, new_chat_bots, new_chat_start_rate,
                user_chat_start_rate, new_user_chat_start_rate,
                experiment_name
            )
            ...
            SELECT
                ...
                CASE WHEN COUNT(DISTINCT user_id) = 0 THEN 0
                     ELSE ROUND(
                         COUNT(DISTINCT CASE WHEN chat_bot IS NOT NULL THEN user_id END) * 1.0 /
                         COUNT(DISTINCT user_id)
                     , 4)
                END AS user_chat_start_rate,
                CASE WHEN COUNT(DISTINCT CASE WHEN is_new_user = 1 THEN user_id END) = 0 THEN 0
                     ELSE ROUND(
                         COUNT(DISTINCT CASE WHEN is_new_user = 1 AND chat_bot IS NOT NULL THEN user_id END) * 1.0 /
