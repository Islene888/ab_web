...
(or will be if needed):
   block data,byte,call,character,Item,complex,contains,data,
...
Note: 'Item' statement is extended: dimensions are moved to variable definitions
 ...
 *** it is a list of dictionaries containing `blocks':
     B = {'block','body','vars','parent_block'[,'name','prefix','args','result',
          'implicit','externals','interfaced','Item','sortvars',
          'commonvars','note']}
     ...
     B['Item'] --- dictionary of Item blocks (list of objects)
     B['commonvars'] --- list of variables used in Item blocks (dimensions are moved to variable definitions)
...
commonpattern = re.compile(
    beforethisafter % ('', 'Item', 'Item', '.*'), re.I), 'Item'
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        cl.append([bn, ol])
        commonkey = {}
        if 'Item' in groupcache[groupcounter]:
            commonkey = groupcache[groupcounter]['Item']
        for c in cl:
        ...
        groupcache[groupcounter]['Item'] = commonkey
        previous_context = ('Item', bn, groupcounter)
    elif case == 'use':
        m1 = re.match(
...
def analyzecommon(block):
    if not hascommon(block):
        return block
    commonvars = []
    for k in list(block['Item'].keys()):
        comvars = []
        for e in block['Item'][k]:
            m = re.match(
            ...
            if m:
                ...
                n = e
                errmess(
                    'analyzecommon: failed to extract "<name>[(<dims>)]" from "%s" in Item /%s/.\n' % (e, k))
            comvars.append(n)
        block['Item'][k] = comvars
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    common = ''
    if 'Item' in block:
        common = common2fortran(block['Item'], tab + tabchar)
