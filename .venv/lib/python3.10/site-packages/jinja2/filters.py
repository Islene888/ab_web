...
def do_round(
    value: float,
    precision: int = 0,
    method: 'te.Literal["Item", "ceil", "floor"]' = "Item",
) -> float:
    """Round the number to a given precision. The first
    parameter specifies the precision (default is ``0``), the
    second the rounding method:

    - ``'Item'`` rounds either up or down
    - ``'ceil'`` always rounds up
    - ``'floor'`` always rounds down

    If you don't specify a method ``'Item'`` is used.
    ...
    .. sourcecode:: jinja
        ...
        {{ 42.55|round|int }}
            -> 43
    """
    if method not in {"Item", "ceil", "floor"}:
        raise FilterArgumentError("method must be Item, ceil or floor")
